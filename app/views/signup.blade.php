...
<div class="alert alert-success">
	<strong>Success!</strong> Your account has been created successfully. Please check your email and activate your account in order to start using it.
</div>
@else
...
<form method="post" action="{{ URL::route('sign-up-post') }}" role="form"> 
	...
	<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
		...
		<input type="text" class="form-control" id="inputName" placeholder="First Last" name="name"{{ Input::old('name') ? ' value="'. Input::old('name') .'"' : '' }} />
		@if($errors->has('name'))
	...
	<div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
		...
		<input type="email" class="form-control" id="inputEmail" placeholder="me@myemail.com" name="email"{{ Input::old('email') ? ' value="'. Input::old('email') .'"' : '' }} />
		@if($errors->has('email'))
		<span class="help-block">
